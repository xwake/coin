...
import urllib,time,random,re,threading,string, sys,os, datetime, thread
from time import sleep


web_site_count=14   #要抓取的网站数目
day_keep=2          #删除数据库中保存时间大于day_keep天的 无效代理
indebug=0        
daemonSubTimeOut = 60    #此处为子线程的生存时限，统一为此处设置

thread_num=100                   # 开 thread_num 个线程检查代理
check_in_one_call=thread_num*10  # 本次程序运行时 最多检查的代理个数


skip_check_in_hour=8    # 在时间 skip_check_in_hour内,不对同一个代理地址再次验证
skip_get_in_hour=24      # 每次采集新代理的最少时间间隔 (小时)
...
#dbfile= cur_path.replace('proxy_cron','blog')+'/proxy/proxier0224.db'     #数据库文件名
#print dbfile

#target_url="http://www.baidu.com/"   # 验证代理的时候通过代理访问这个地址
#target_string="030173"               # 如果返回的html中包含这个字符串，百度必含字段：京ICP证030173号

target_url="http://www.douban.com/"   
target_string="0110418"
...
def output_file():
    ...
    for i in range(fnum):
        ...
        f=open('proxy/' + output_filename[i]+"."+output_type,'w')
...
def check_one_proxy(ip,port):
    ...
    #print '-- proxyUpdate.py > ',len(update_array),' of ',check_in_one_call," ",ip,':',port,'--',str(int(timeused))+'s', '<',active,'>'
...
def build_list_urls_14(page=9):
	ret=[]
	for i in range(1,page+1):
		ret.append('http://www.cnproxy.com/proxy%(n)01d.html'%{'n':i})		
    ret.append('http://www.cnproxy.com/proxy10.html')	
	return ret
...
def parse_page_14(html=''):
	...
	for match in matches:
        ...
        port = port.replace('+','').replace('c','1').replace('a','2').replace('v','3').replace('m','4').replace('b','5')
        port = port.replace('w','6').replace('i','7').replace('r','8').replace('l','9').replace('q','0')
		ret.append([ip,port,type,area])
        if indebug: print '10',ip,port,type,area
	return ret
...
def check_proxy(index,checklist=[]):
    ...
    import thread
    format_start = '%s:%s.%s'%(start.minute,start.second,start.microsecond)
    format_end = '%s:%s.%s'%(end.minute,end.second,end.microsecond)
    #sleep(10)
    #print '--proxyUpdate.py > 耗',(end-start),'s', threading.active_count(),'(',thread.get_ident(),')',format_start,'-',format_end

#守护子线程，有规定的结束时间，让其所有孙线程随之结束
def daemonSub (tHandles,timeout=30):
    print '--proxyUpdate.py > daemonSub() is start.',threading.active_count(),'/',datetime.datetime.now()
    ...
    #sleep(timeout)
    print '--proxyUpdate.py > daemonSub() is finish.',threading.active_count(),'/',datetime.datetime.now()
...
def patch_check_proxy(threadCount,action=''):
    ...
    sql="""
           select `ip`,`port` FROM `proxier` where
                 `time_checked` < (strftime('%s','now')-""" + str(skip_check_in_hour*3600) + """) 
                 and """ + strwhere + """ 
            	 order by """+orderby+""" 
            	 limit """+str(check_in_one_call)+""" 
        """
    #print '--proxyUpdate.py > ',sql
    conn.execute(sql)
    rows = conn.fetchall()   
    ...
    print "--proxyUpdate.py > 现在开始验证以下代理服务器....."
    for index in range(14,threadCount):        
    ...
    print '--proxyUpdate.py > daemonThread.alive:',daemonThread.is_alive(),'/',daemonThread.ident,threading.active_count(),datetime.datetime.now()
    tcount = 0
    fcount = 0
    for th in threads:
        if th.isAlive() == True:
    ...
    print '--proxyUpdate.py > alive:',tcount,'+',fcount,'=',len(threads)
    print '--proxyUpdate.py > 分配检查线程完成'
    '''
    for thread in threads:
        import datetime
        #print '--proxyUpdate.py >', datetime.datetime.now(),thread
        start = datetime.datetime.now()
        thread.join(2)    
        end = datetime.datetime.now()
        #print '--proxyUpdate.py >', datetime.datetime.now(),thread
    print '--proxyUpdate.py > join()线程完成'
    '''
    #time.sleep(5)
...
def get_proxy_one_website(index):
    ...
    urls=eval(func+'()')
    print '--proxyUpdate.py > get_proxy_one_website.urls.size: ' +  str(len(urls)) +'-'+ str(index)
...
def get_all_proxies():
 ...
 如果一定要现在抓取代理，请修改全局变量: skip_get_in_hour 的值
    ...
    print "--proxyUpdate.py > 现在开始从以下"+str(web_site_count)+"个网站抓取代理列表...."
    threads=[]
...
def add_proxies_to_db():
    ...
    for i in range(count):
        ...
        ('"""+item[0]+"',"+item[1]+","+item[2]+",strftime('%s','now'),'"+clean_string(item[3])+"')"
        sql = unicode(sql,"gbk")
        try:
            conn.execute(sql)
            #if indebug == 1 : print "%(num)2.1f\%\t"%{'num':100*(i+1)/count},item[0],":",item[1]
        except:
            pass 
...
def update_proxies():
    ...
    for item in update_array:
        sql='''
             update `proxier` set `time_checked`=%(time_checked)s, 
                `active`=%(active)01d, 
                 `speed`=%(speed)02.3f                 
                 where `ip`='%(ip)01s' and `port`=%(port)01d                            
            '''%{'active':item[2],'speed':item[3],'ip':item[0],'port':item[1],'time_checked':"strftime('%s','now')"}
        #print sql
        #conn.execute(sql)  
        ...
        try:
            ...
            print '--proxyUpdate.py > update_proxies() is error'
            pass
    print '--proxyUpdate.py > success check proxier %s rows'% i
...
def open_database():
    ...
    try:
        ...
        print "--proxyUpdate.py > 操作sqlite数据库失败，请确保脚本所在目录具有写权限"
        raise SystemExit
    ...
    conn.execute("""DELETE FROM `proxier`
                        where `time_added`< (strftime('%s','now')-?) 
